...
import com.mahdiparastesh.migratio.data.Criterion
import com.mahdiparastesh.migratio.data.MyCriterion
        ...
        fun compute(
            ...
            for (mycon in myCons) {
                ...
                for (mycri in myCris.indices) {
                    val notReadyVal =
                        compileValue(myCon, findCriByTag(myCris[mycri].tag, cris)!!, cons)
                    val readyVal =
                        if (myCris[mycri].good != "-" && myCris[mycri].good != "+")
                            abs(notReadyVal - safeGood(myCris[mycri], cris))
                        else notReadyVal
                    scoresNoImport.add(
                        relativeScore(minAndMax[mycri][0], minAndMax[mycri][1], readyVal)
                    )
                }
        ...
        fun compileValue(myCon: Country, myCri: Criterion, cons: List<Country>): Double {
            ...
            when {
                value == "-" -> {
                    ...
                    for (ss in split)
                        estimation.add(
                            compileValue(findConByTag(ss, cons)!!, myCri, cons)
                        )
