...
import com.mahdiparastesh.migratio.Fun.Companion.countryNames
import java.util.*
        ...
        class SortCon(val by: Int = 0) : Comparator<Country> {
            override fun compare(a: Country, b: Country) = when (by) {
                1 -> Collator.getInstance(Locale("fa")).compare(
                    countryNames()[com.mahdiparastesh.migratio.Countries.TAGS.indexOf(a.tag)],
                    countryNames()[com.mahdiparastesh.migratio.Countries.TAGS.indexOf(b.tag)]
                )
