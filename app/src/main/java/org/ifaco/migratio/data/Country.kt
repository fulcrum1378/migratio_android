...
import org.ifaco.migratio.Countries
import org.ifaco.migratio.Fun.Companion.countryNames
import java.util.*
        ...
        class SortCon(val by: Int = 0) : Comparator<Country> {
            override fun compare(a: Country, b: Country) = when (by) {
                1 -> Collator.getInstance(Locale("fa")).compare(
                    countryNames()[Countries.TAGS.indexOf(a.tag)],
                    countryNames()[Countries.TAGS.indexOf(b.tag)]
                )
